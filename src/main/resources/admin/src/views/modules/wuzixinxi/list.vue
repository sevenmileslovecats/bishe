...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('wuzixinxi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('wuzixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('wuzixinxi','库存数量')" type="success" @click="chartDialogShow1">
		...
		<wuzishenling-cross-add-or-update v-if="wuzishenlingCrossAddOrUpdateFlag" :parent="this" ref="wuzishenlingCrossaddOrUpdate"></wuzishenling-cross-add-or-update>
		<chukufenbo-cross-add-or-update v-if="chukufenboCrossAddOrUpdateFlag" :parent="this" ref="chukufenboCrossaddOrUpdate"></chukufenbo-cross-add-or-update>
...
<script>
	...
	import wuzishenlingCrossAddOrUpdate from "../wuzishenling/add-or-update";
	import chukufenboCrossAddOrUpdate from "../chukufenbo/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				wuzishenlingCrossAddOrUpdateFlag: false,
				chukufenboCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			wuzishenlingCrossAddOrUpdate,
			chukufenboCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			tableRowClassName({
			  ...
			  rowIndex
				...
				let title = ''
				title = 'wuzimingcheng'
				if(this.changeInList(row[title],this.baozhiqiRemind)) {
					name = name + ' ' + 'baozhiqi-row';
				}
				if((Number('0')<Number('100')&&row.wuzishuliang>=Number('0')&&row.wuzishuliang<=Number('100'))||(Number('0')>Number('100')&&(row.wuzishuliang>=Number('0')||row.wuzishuliang<=Number('100')))) {
					name = name + ' ' + 'wuzishuliang-row';
				}
				return name;
			},
			changeClick(e) {
				// 提醒标题有吗
				this.searchForm.wuzimingcheng = e
				this.search()
				// 关闭弹窗
				this.$msgbox.close()
			},
			...
			wuzishenlingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.wuzishenlingCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.wuzishenlingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.wuzishenlingCrossaddOrUpdate.init(row.id,type);
				});
			},
			chukufenboCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.chukufenboCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','wuzixinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chukufenboCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chukufenboCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			init () {
				...
				let urlwuzishuliang = 'wuzixinxi/remind/wuzishuliang/1';
				urlwuzishuliang+=`?remindend=100`,

				urlwuzishuliang+=`&remindstart=0`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						var name = data.data
						this.wuzishuliangRemind = data.data
						var arr = []
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `${data.count}条物资数量预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `库存不足,请尽快补充！`),
				...
				let urlbaozhiqi = 'wuzixinxi/remind/baozhiqi/2';
				urlbaozhiqi+=`?remindend=5`,

				urlbaozhiqi+=`&remindstart=0`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `${data.count}条保质期到期预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `临近过期,请尽快处理！`),
									h('div', { style: 'text-align: center' }, [
										h('span', null, `包含：`),
										arr
									])
...
<style lang="scss" scoped>
	...
	/deep/ .el-table .wuzishuliang-row .showColor1 {
		background-color: #f7bba6 !important;
		color: #000;
	}
	/deep/ .el-table .baozhiqi-row .showColor2 {
		background-color: #f4d899 !important;
		color: #000;
	}
