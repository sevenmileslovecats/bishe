...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="异议反馈" prop="yiyifankui" >
					<el-input v-model="ruleForm.yiyifankui" placeholder="异议反馈" clearable  :readonly="ro.yiyifankui"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="异议反馈" prop="yiyifankui" >
					<el-input v-model="ruleForm.yiyifankui" placeholder="异议反馈" readonly></el-input>
				</el-form-item>
				<el-form-item class="upload" v-if="type!='info' && !ro.zuozhengtupian" label="佐证图片" prop="zuozhengtupian" >
					<file-upload
						tip="点击上传佐证图片"
						action="file/upload"
						:limit="3"
						:disabled="ro.zuozhengtupian"
						:multiple="true"
						:fileUrls="ruleForm.zuozhengtupian?ruleForm.zuozhengtupian:''"
						@change="zuozhengtupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.zuozhengtupian" label="佐证图片" prop="zuozhengtupian" >
					<img v-if="ruleForm.zuozhengtupian.substring(0,4)=='http'&&ruleForm.zuozhengtupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.zuozhengtupian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.zuozhengtupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.zuozhengtupian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zuozhengtupian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				<el-form-item class="fileupload" v-if="type!='info'&& !ro.zuozhengcailiao" label="佐证材料" prop="zuozhengcailiao" >
					<file-upload
						tip="点击上传佐证材料"
						action="file/upload"
						:limit="1"
						:type="2"
						:multiple="true"
						:disabled="ro.zuozhengcailiao"
						:fileUrls="ruleForm.zuozhengcailiao?ruleForm.zuozhengcailiao:''"
						@change="zuozhengcailiaoUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item v-else-if="ruleForm.zuozhengcailiao" label="佐证材料" prop="zuozhengcailiao" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.zuozhengcailiao)">
	...
	export default {
		data() {
			...
			return {
				...
				ro:{
					yiyifankui : false,
					zuozhengtupian : false,
					zuozhengcailiao : false,
					tijiaoshijian : false,
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='yiyifankui'){
							this.ruleForm.yiyifankui = obj[o];
							this.ro.yiyifankui = true;
							continue;
						}
						if(o=='zuozhengtupian'){
							this.ruleForm.zuozhengtupian = obj[o];
							this.ro.zuozhengtupian = true;
							continue;
						}
						if(o=='zuozhengcailiao'){
							this.ruleForm.zuozhengcailiao = obj[o];
							this.ro.zuozhengcailiao = true;
							continue;
						}
						if(o=='tijiaoshijian'){
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.ruleForm.zuozhengtupian!=null) {
								this.ruleForm.zuozhengtupian = this.ruleForm.zuozhengtupian.replace(new RegExp(this.$base.url,"g"),"");
							}
							if(this.ruleForm.zuozhengcailiao!=null) {
								this.ruleForm.zuozhengcailiao = this.ruleForm.zuozhengcailiao.replace(new RegExp(this.$base.url,"g"),"");
							}
							...
							await this.$http({
								url: `yiyifankui/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yiyifankuiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			zuozhengtupianUploadChange(fileUrls) {
				this.ruleForm.zuozhengtupian = fileUrls;
			},
			zuozhengcailiaoUploadChange(fileUrls) {
				this.ruleForm.zuozhengcailiao = fileUrls;
			},
