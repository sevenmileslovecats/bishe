	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.qianshoutupian" label="签收图片" prop="qianshoutupian" >
					<file-upload
						tip="点击上传签收图片"
						action="file/upload"
						:limit="3"
						:disabled="ro.qianshoutupian"
						:multiple="true"
						:fileUrls="ruleForm.qianshoutupian?ruleForm.qianshoutupian:''"
						@change="qianshoutupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.qianshoutupian" label="签收图片" prop="qianshoutupian" >
					<img v-if="ruleForm.qianshoutupian.substring(0,4)=='http'&&ruleForm.qianshoutupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.qianshoutupian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.qianshoutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.qianshoutupian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qianshoutupian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.qianshoushijian" label="签收时间" prop="qianshoushijian" >
					<el-input v-model="ruleForm.qianshoushijian" placeholder="签收时间" readonly></el-input>
				</el-form-item>
			</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='qianshoutupian'){
							this.ruleForm.qianshoutupian = obj[o];
							this.ro.qianshoutupian = true;
							continue;
						}
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							if(this.ruleForm.qianshoutupian!=null) {
								this.ruleForm.qianshoutupian = this.ruleForm.qianshoutupian.replace(new RegExp(this.$base.url,"g"),"");
							}
							...
							await this.$http({
								url: `jieshouxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jieshouxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			qianshoutupianUploadChange(fileUrls) {
				this.ruleForm.qianshoutupian = fileUrls;
			},
		}
	};
</script>
