...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('jieshouxinxi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('jieshouxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","boxShadow":"none","borderColor":"#ff9164","borderRadius":"10px","background":"#fff","borderWidth":"4px 0 0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="qianshoutupian" width="200" label="签收图片">
						<template slot-scope="scope">
							<div v-if="scope.row.qianshoutupian">
								<img v-if="scope.row.qianshoutupian.substring(0,4)=='http'&&scope.row.qianshoutupian.split(',w').length>1" :src="scope.row.qianshoutupian" width="100" height="100" style="object-fit: cover" @error="$event.target.style.display='none'" @click="imgPreView(scope.row.qianshoutupian)">
								<img v-else-if="scope.row.qianshoutupian.substring(0,4)=='http'" :src="scope.row.qianshoutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @error="$event.target.style.display='none'" @click="imgPreView(scope.row.qianshoutupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.qianshoutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @error="$event.target.style.display='none'" @click="imgPreView($base.url+scope.row.qianshoutupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jieshouxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('jieshouxinxi','使用反馈')" @click="shiyongfankuiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								使用反馈
							</el-button>
							<el-button class="edit" v-if=" isAuth('jieshouxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jieshouxinxi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<shiyongfankui-cross-add-or-update v-if="shiyongfankuiCrossAddOrUpdateFlag" :parent="this" ref="shiyongfankuiCrossaddOrUpdate"></shiyongfankui-cross-add-or-update>
...
<script>
	...
	import shiyongfankuiCrossAddOrUpdate from "../shiyongfankui/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				shiyongfankuiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			shiyongfankuiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shiyongfankuiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jieshouxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shiyongfankuiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shiyongfankuiCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
			...
			getDataList() {
				...
				params['sort'] = 'qianshoushijian';
				params['order'] = 'desc';
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "jieshouxinxi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
