					...
					<el-table-column  :resizable='true' prop="qianshoutupian" width="200" label="签收图片">
						<template slot-scope="scope">
							<div v-if="scope.row.qianshoutupian">
								<img v-if="scope.row.qianshoutupian.substring(0,4)=='http'&&scope.row.qianshoutupian.split(',w').length>1" :src="scope.row.qianshoutupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.qianshoutupian)">
								<img v-else-if="scope.row.qianshoutupian.substring(0,4)=='http'" :src="scope.row.qianshoutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.qianshoutupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.qianshoutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.qianshoutupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
