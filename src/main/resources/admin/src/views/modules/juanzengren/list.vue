		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('juanzengren','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('juanzengren','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-table-column :resizable='true' :sortable='true' prop="status" label="状态">
						<template slot-scope="scope">
							<el-switch
								...
								active-color="#ff8b00"
								inactive-color="#ccc"
								@change="(e)=>juanzengrenstatusChange(e,scope.row)">
							</el-switch>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('juanzengren','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('juanzengren','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('juanzengren','删除')" type="primary" @click="deleteHandler(scope.row.id)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			juanzengrenstatusChange(e,row){
				if(row.status==0){
					row.passwordwrongnum = 0
				}
				this.$http({
					url:'juanzengren/update',
					method:'post',
					data:row
				}).then(res=>{
					if(row.status==1){
