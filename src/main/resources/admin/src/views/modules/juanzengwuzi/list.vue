...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px 20px 10px","boxShadow":"none","borderColor":"#ff9164","borderRadius":"10px","flexWrap":"wrap","background":"#fff","borderWidth":"4px 0 0 0","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					...
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}' class="select" label="物资种类" prop="wuzizhonglei">
						...
						<el-select clearable v-model="searchForm.wuzizhonglei" placeholder="请选择物资种类" >
							<el-option v-for="(item,index) in wuzizhongleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}' class="select" label="验收状态" prop="yanshouzhuangtai">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">验收状态</label>
						<el-select clearable v-model="searchForm.yanshouzhuangtai" placeholder="请选择验收状态" >
							<el-option v-for="(item,index) in yanshouzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('juanzengwuzi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('juanzengwuzi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('juanzengwuzi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
					...
					<el-button class="btn18" v-if="isAuth('juanzengwuzi','日捐赠数量')" type="success" @click="chartDialogShow1">
					...
					<el-button class="btn18" v-if="isAuth('juanzengwuzi','月捐赠数量')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont icon-xihuan" :style='{"padding":"10px","margin":"0 2px","color":"#af7d59","borderRadius":"100%","background":"#e9cfbc20","fontSize":"20px","fontWeight":"500","height":"40px"}'></span>
						月捐赠数量
					</el-button>
					<el-button class="btn18" v-if="isAuth('juanzengwuzi','物资类型分布')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont icon-xihuan" :style='{"padding":"10px","margin":"0 2px","color":"#af7d59","borderRadius":"100%","background":"#e9cfbc20","fontSize":"20px","fontWeight":"500","height":"40px"}'></span>
						物资类型分布
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"20px","boxShadow":"none","borderColor":"#ff9164","borderRadius":"10px","background":"#fff","borderWidth":"4px 0 0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('juanzengwuzi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('juanzengwuzi','颁发证书')" @click="juanzengzhengshuCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','颁发')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								颁发证书
							</el-button>
							<el-button class="btn8" v-if="isAuth('juanzengwuzi','验收')" @click="yanshoujiluCrossAddOrUpdateHandler(scope.row,'cross','是','','yanshouzhuangtai','已验收','已验收,未验收'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								验收
							</el-button>
							<el-button class="edit" v-if=" isAuth('juanzengwuzi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('juanzengwuzi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<juanzengzhengshu-cross-add-or-update v-if="juanzengzhengshuCrossAddOrUpdateFlag" :parent="this" ref="juanzengzhengshuCrossaddOrUpdate"></juanzengzhengshu-cross-add-or-update>
		<yanshoujilu-cross-add-or-update v-if="yanshoujiluCrossAddOrUpdateFlag" :parent="this" ref="yanshoujiluCrossaddOrUpdate"></yanshoujilu-cross-add-or-update>
...
<script>
	...
	import juanzengzhengshuCrossAddOrUpdate from "../juanzengzhengshu/add-or-update";
	import yanshoujiluCrossAddOrUpdate from "../yanshoujilu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				juanzengzhengshuCrossAddOrUpdateFlag: false,
				yanshoujiluCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			juanzengzhengshuCrossAddOrUpdate,
			yanshoujiluCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			juanzengzhengshuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.juanzengzhengshuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','juanzengwuzi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.juanzengzhengshuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.juanzengzhengshuCrossaddOrUpdate.init(row.id,type);
				});
			},
			yanshoujiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "warning",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yanshoujiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','juanzengwuzi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yanshoujiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yanshoujiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "juanzengwuzi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
