		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"20px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('shiyongfankui','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('shiyongfankui','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-table-column  :resizable='true' prop="changjingzhaopian" width="200" label="场景照片">
						<template slot-scope="scope">
							<div v-if="scope.row.changjingzhaopian">
								...
								<img v-else :src="$base.url+scope.row.changjingzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @error="$event.target.style.display='none'" @click="imgPreView($base.url+scope.row.changjingzhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussshiyongfankui',query:{refid:id}});
			},
			// 删除
