		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('chukufenbo','新增')" type="primary" @click="add('/index/chukufenboAdd')">
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-btn5" type="success" @click.native="chapterClick(scope.row)" v-if="btnAuth('chukufenbo','章节管理')">
...
<script>
	...
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'物流状态',list:this.wuliuzhuangtaiOptions,tableName: 'wuliuzhuangtai',check: -1})
			if(this.centerType) {
			}
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.wuliuzhuangtai != ''&&this.formSearch.wuliuzhuangtai != undefined) searchWhere.wuliuzhuangtai = this.formSearch.wuliuzhuangtai;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`chukufenbo/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/chukufenboDetail', query: params});
			},
