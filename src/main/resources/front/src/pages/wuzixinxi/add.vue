...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="存储位置" prop="cunchuweizhi">
				<el-input v-model="ruleForm.cunchuweizhi" 
					placeholder="存储位置" clearable :readonly="ro.cunchuweizhi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="物资详情" prop="wuzixiangqing">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.wuzixiangqing"
					placeholder="物资详情"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`wuzixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`wuzixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
