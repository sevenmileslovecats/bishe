	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/juanzengwuzi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">捐赠时间</div>
						<div class="text "  >{{detail.juanzengshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">验收状态</div>
						<div class="text "  >{{detail.yanshouzhuangtai}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('juanzengwuzi','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('juanzengwuzi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('juanzengwuzi','颁发证书')" 
							@click="onAcross('juanzengzhengshu','是','','[1]','颁发')" type="warning">
							颁发证书
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('juanzengwuzi','验收')" 
							@click="onAcross('yanshoujilu','是','','yanshouzhuangtai','已验收','已验收,未验收'.split(',')[0])" type="warning">
							验收
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('juanzengwuzi','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('juanzengwuzi','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`juanzengwuzi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/juanzengwuzi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/juanzengwuziAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此捐赠物资？') .then(_ => {
					this.$http.post('juanzengwuzi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
