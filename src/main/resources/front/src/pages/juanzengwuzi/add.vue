	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="物资种类" prop="wuzizhonglei">
				<el-select v-model="ruleForm.wuzizhonglei" placeholder="请选择物资种类" :disabled="ro.wuzizhonglei"  filterable>
					<el-option
			...
			<el-form-item class="add-item"  label="验收状态" prop="yanshouzhuangtai">
				<el-select v-model="ruleForm.yanshouzhuangtai" placeholder="请选择验收状态" :disabled="ro.yanshouzhuangtai"  filterable>
					<el-option
						v-for="(item,index) in yanshouzhuangtaiOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yanshouzhuangtai'){
							this.ruleForm.yanshouzhuangtai = obj[o];
							this.ro.yanshouzhuangtai = true;
							continue;
						}
						if(o=='zhanghao'){
					...
					this.ruleForm.yanshouzhuangtai = '未验收'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
				...
				this.$http.get('option/wuzizhonglei/wuzizhonglei', {}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`juanzengwuzi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`juanzengwuzi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
