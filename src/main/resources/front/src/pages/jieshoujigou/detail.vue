	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jieshoujigou?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.jigoujianjie}}</div>
					</div>
					<div class="item">
						<div class="lable">机构地址</div>
						<div class="text "  >{{detail.jigoudizhi}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">机构登记证书</div>
						<el-button class="uploadBtn" @click="download(detail.jigoudengjizhengshu )">点击下载</el-button>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jieshoujigou','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jieshoujigou','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('jieshoujigou','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('jieshoujigou','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jieshoujigou', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jieshoujigouAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此接收机构？') .then(_ => {
					this.$http.post('jieshoujigou/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterjieshoujigou', query: params});
			},
			// 获取uuid
			getUUID() {
