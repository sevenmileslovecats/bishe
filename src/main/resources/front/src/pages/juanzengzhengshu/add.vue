...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="捐赠证书" prop="juanzengzhengshu">
				<file-upload
					tip="点击上传捐赠证书"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="颁发时间" prop="banfashijian">
				<el-date-picker
					:disabled="ro.banfashijian"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`juanzengzhengshu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						...
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('juanzengzhengshu/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`juanzengzhengshu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			juanzengzhengshuUploadChange(fileUrls) {
				this.ruleForm.juanzengzhengshu = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
