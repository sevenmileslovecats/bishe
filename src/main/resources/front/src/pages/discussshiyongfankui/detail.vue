	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussshiyongfankui?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discussshiyongfankui','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('discussshiyongfankui','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('discussshiyongfankui','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/discussshiyongfankuiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此使用反馈评论？') .then(_ => {
					this.$http.post('discussshiyongfankui/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
