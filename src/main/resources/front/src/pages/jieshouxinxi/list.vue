		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('jieshouxinxi','新增')" type="primary" @click="add('/index/jieshouxinxiAdd')">
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column :resizable='true' prop="qianshoutupian" width="200" label="签收图片">
						<template slot-scope="scope">
							<div v-if="scope.row.qianshoutupian">
								...
								<img v-else :src="baseUrl+scope.row.qianshoutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @error="$event.target.style.display='none'" @click="imgPreView(baseUrl+scope.row.qianshoutupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-btn5" type="success" @click.native="chapterClick(scope.row)" v-if="btnAuth('jieshouxinxi','章节管理')">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`jieshouxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jieshouxinxiDetail', query: params});
			},
			...
			chapterClick (row){
				...
				this.$router.push({path: '/index/chapterjieshouxinxi', query: params});
			},
		}
	}
</script>
