...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="签收图片" v-if="type!='cross' || (type=='cross' && !ro.qianshoutupian)" prop="qianshoutupian">
				<file-upload
					tip="点击上传签收图片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="签收图片" prop="qianshoutupian">
				<img v-if="ruleForm.qianshoutupian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.qianshoutupian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.qianshoutupian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="签收时间" prop="qianshoushijian">
				<el-date-picker
					:disabled="ro.qianshoushijian"
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='qianshoutupian'){
							this.ruleForm.qianshoutupian = obj[o]?obj[o].split(",")[0]:'';
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`jieshouxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jieshouxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			qianshoutupianUploadChange(fileUrls) {
				this.ruleForm.qianshoutupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
