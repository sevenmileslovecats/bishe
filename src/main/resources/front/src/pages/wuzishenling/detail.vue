	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/wuzishenling?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">物资规格</div>
						<div class="text "  >{{detail.wuziguige}}</div>
					</div>
					<div class="item">
						<div class="lable">申领数量</div>
						<div class="text "  >{{detail.shenlingshuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">申领时间</div>
						<div class="text "  >{{detail.shenlingshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">出库状态</div>
						<div class="text "  >{{detail.chukuzhuangtai}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">物资申领表</div>
						<el-button class="uploadBtn" @click="download(detail.wuzishenlingbiao )">点击下载</el-button>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('wuzishenling','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('wuzishenling','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('wuzishenling','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('wuzishenling','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/wuzishenling', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/wuzishenlingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此物资申领？') .then(_ => {
					this.$http.post('wuzishenling/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
