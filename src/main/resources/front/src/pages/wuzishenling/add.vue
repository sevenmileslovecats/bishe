	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="申领编号" prop="shenlingbianhao">
				<el-input v-model="ruleForm.shenlingbianhao" placeholder="申领编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="物资规格" prop="wuziguige">
			...
			<el-form-item class="add-item" label="申领数量" prop="shenlingshuliang">
				<el-input v-model.number="ruleForm.shenlingshuliang" 
					placeholder="申领数量" clearable :readonly="ro.shenlingshuliang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="物资申领表" prop="wuzishenlingbiao">
				<file-upload
					tip="点击上传物资申领表"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="申领时间" prop="shenlingshijian">
				<el-date-picker
					:disabled="ro.shenlingshijian"
			...
			<el-form-item class="add-item"  label="出库状态" prop="chukuzhuangtai">
				<el-select v-model="ruleForm.chukuzhuangtai" placeholder="请选择出库状态" :disabled="ro.chukuzhuangtai"  filterable>
					<el-option
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.chukuzhuangtai = '未出库'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.quyu!=''&&json.quyu) || json.quyu==0){
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`wuzishenling/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`wuzishenling/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			wuzishenlingbiaoUploadChange(fileUrls) {
				this.ruleForm.wuzishenlingbiao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
