		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('wuzishenling','新增')" type="primary" @click="add('/index/wuzishenlingAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('wuzishenling','机构分拨占比')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						机构分拨占比
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('wuzishenling','区域需求热度')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						区域需求热度
					</el-button>
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-btn5" type="success" @click.native="chapterClick(scope.row)" v-if="btnAuth('wuzishenling','章节管理')">
...
<script>
	...
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'出库状态',list:this.chukuzhuangtaiOptions,tableName: 'chukuzhuangtai',check: -1})
			if(this.centerType) {
			}
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.chukuzhuangtai != ''&&this.formSearch.chukuzhuangtai != undefined) searchWhere.chukuzhuangtai = this.formSearch.chukuzhuangtai;
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				...
				this.$http.get(`wuzishenling/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/wuzishenlingDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http.get("wuzishenling/group/jigoumingcheng",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							var option = {};
							let titleObj = this.pie3.title
							titleObj.text = '机构分拨占比'
							
							const legendObj = this.pie3.legend
							...
							seriesObj = Object.assign(seriesObj , this.pie3.series)
							const gridObj = this.pie3.grid
							option = {
								backgroundColor: this.pie3.backgroundColor,
								color: this.pie3.color,
								title: titleObj,
								legend: legendObj,
								grid: gridObj,
								tooltip: tooltipObj,
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					this.$http.get("wuzishenling/group/quyu",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chapterClick (row){
				...
				this.$router.push({path: '/index/chapterwuzishenling', query: params});
			},
		}
	}
</script>
