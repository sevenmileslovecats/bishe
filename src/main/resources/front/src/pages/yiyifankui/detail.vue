	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yiyifankui?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">异议反馈</div>
						<div class="text "  >{{detail.yiyifankui}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">佐证材料</div>
						<el-button class="uploadBtn" @click="download(detail.zuozhengcailiao )">点击下载</el-button>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yiyifankui','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yiyifankui','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yiyifankui','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('yiyifankui','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						if(this.detail.zuozhengtupian) {
							this.detailBanner = this.detail.zuozhengtupian.split(",w").length>1?[this.detail.zuozhengtupian]:this.detail.zuozhengtupian.split(',');
						}
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yiyifankui', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yiyifankuiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此异议反馈？') .then(_ => {
					this.$http.post('yiyifankui/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
