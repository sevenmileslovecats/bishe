...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="异议反馈" prop="yiyifankui">
				<el-input v-model="ruleForm.yiyifankui" 
					placeholder="异议反馈" clearable :readonly="ro.yiyifankui"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="佐证图片" v-if="type!='cross' || (type=='cross' && !ro.zuozhengtupian)" prop="zuozhengtupian">
				<file-upload
					tip="点击上传佐证图片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="佐证图片" prop="zuozhengtupian">
				<img v-if="ruleForm.zuozhengtupian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.zuozhengtupian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.zuozhengtupian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="佐证材料" prop="zuozhengcailiao">
				<file-upload
					tip="点击上传佐证材料"
					action="file/upload"
					:limit="1"
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='zuozhengtupian'){
							this.ruleForm.zuozhengtupian = obj[o]?obj[o].split(",")[0]:'';
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`yiyifankui/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yiyifankui/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			zuozhengtupianUploadChange(fileUrls) {
				this.ruleForm.zuozhengtupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
			zuozhengcailiaoUploadChange(fileUrls) {
				this.ruleForm.zuozhengcailiao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
